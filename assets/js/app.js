const App = {
    ...
    return `
      <div class="flex items-center justify-center min-h-screen p-6 bg-white">
        <div class="w-full max-w-sm p-4 text-center">
          ...
          <h2 class="text-4xl font-extrabold mb-1 tracking-tighter text-gray-900">SIPANDA</h2>
  ...
  async handleLogin() {
    ...
    if (!u || !p) return this.showToast("Lengkapi data login!", "error");

    // Integrasi API (Sinkronisasi dengan backend)
    try {
      this.showToast("Memverifikasi Keamanan...", "success");
      // Ganti ini dengan: const res = await API.login(u, p);
      // Untuk demo, kita simulasikan:
      const role = u.toLowerCase() === 'admin' ? 'Admin' : 'Staff';
      this.user = { User_ID: u, Name: u.toUpperCase(), Role: role, Status: "Active" };
  ...
  showToast(m) {
    let c = document.querySelector('.toast-container') || document.createElement('div');
    c.className = 'toast-container'; document.body.appendChild(c);
    const t = document.createElement('div'); t.className = 'toast-item border-l-4 border-red-500';
    t.innerHTML = `<span>${m}</span>`; c.appendChild(t); setTimeout(() => t.remove(), 3000);
  },
  startClock() {
